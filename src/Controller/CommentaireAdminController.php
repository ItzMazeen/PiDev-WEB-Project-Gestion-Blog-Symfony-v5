...
//DELETE COMMENT BY ID API :
    #[Route('/removeCommentAPI/{id}', name: 'app_comment_deleteJSON')]
    public function deleteAPI(Request $req,$id,NormalizerInterface $normalizer): Response
    {
        ...
        $em->flush();
        $articlesNormalizer = $normalizer->normalize($commentaire, 'json' , ['groups' => "articles"]);
        $json = json_encode($articlesNormalizer);
        return new Response("Comment deleted sucesseflly" . $json);
    }

//UPDATE COMMENT BY ID API :
#[Route('/updateCommentAPI/{id}', name: 'app_comment_updateJSON')]
public function updateAPI(Request $req,$id,NormalizerInterface $normalizer): Response
{
    ...
    $articlesNormalizer = $normalizer->normalize($commentaire, 'json' , ['groups' => "articles"]);
    $json = json_encode($articlesNormalizer);
    return new Response("comment updated sucesseflly" . $json);
}
}
